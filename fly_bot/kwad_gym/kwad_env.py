...
class KwadEnv(robot_gazebo_env.RobotGazeboEnv):
    def __init__(self):
        ...
        # Does not have any accessibles
        self.controllers_list = []

        # Does not use namespace
        self.robot_name_space = ""

        super(KwadEnv, self).__init__(controllers_list=self.controllers_list,
        ...
        # ToDo probably change message type
        self.kwad_control_publisher = rospy.Publisher("Kwad_gym/action",
                                                      Float64MultiArray,
                                                      queue_size=20)
        ...
        rospy.logdebug("Finished KwadEnv INIT...")
    ...
    def _init_camera(self):
        ...
        byte_size = 4
        overhead = 2  # reserving memory for ros header
        buff_size = img_dims * byte_size * overhead
    ...
    def _check_publishers_connection(self):
        ...
        while self.kwad_control_publisher.get_num_connections() == 0 and not rospy.is_shutdown():
            rospy.logdebug("No susbribers to kwad_control_publisher yet so we wait and try again")
            try:
                rate.sleep()
            except rospy.ROSInterruptException:
                # This is to avoid error when world is rested, time when backwards.
                pass
        rospy.logdebug("kwad_control_publisher Publisher Connected")
    ...
    def _create_setpoint_command(self, roll, pitch, yaw):
        command = np.array(pitch, roll, yaw)
        self.kwad_control_publisher.publish(command)
