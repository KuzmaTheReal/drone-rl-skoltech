...
def get_setpoints(msg):
	f = Float64MultiArray()


def control_kwad(msg, args):
	# Declare global variables as you dont want these to die, reset to zero and then re-initiate when the function
	# is called again.
	global roll, pitch, yaw, err_roll, err_pitch, err_yaw, x_pos, y_pos, z_pos
	...
	# In the simulation, there are objects like: ground, Contruction_cone, quadcopter (named as 'Kwad') etc.
	...
	# Alternatively, you can add your 'control-file' with other algorithms such as Reinforcement learning,
	# and import the main function here instead of PID().

	(fUpdated, err_roll, err_pitch, err_yaw) = PID(roll, pitch, yaw, f, z_pos, setpoint_pitch=0, setpoint_roll=0, setpoint_yaw=0)
	
	# The object args contains the tuple of objects (velPub, err_rollPub, err_pitchPub, err_yawPub.
...
# initiate publishers that publish errors (roll, pitch,yaw - setpoint) so that it can be plotted
# via rqt_plot /err_<name>
...
# Upon receiving the messages, the objects msg, velPub, err_rollPub, err_pitchPub and err_yawPub
...
setpointSub = rospy.Subscriber('Kwad_gym/action', np.array, get_setpoints)
